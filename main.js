...
function ensureScrollWrapper(table) {
  ...
  if (!parent || parent.classList.contains("tcr-scroll"))
    return;
  const wrapper = document.createElement("div");
  wrapper.className = "tcr-scroll";
  parent.insertBefore(wrapper, table);
  wrapper.appendChild(table);
}
function pinAllColumns(table, headerCells, colCount, applyWidth, savedWidths) {
  const widths = new Array(colCount);
  for (let i = 0; i < colCount; i++) {
    const saved = savedWidths ? savedWidths[i] : void 0;
    widths[i] = saved !== void 0 && saved > 0 ? saved : headerCells[i].getBoundingClientRect().width;
  }
  for (let i = 0; i < colCount; i++) {
    if (widths[i] > 0)
      applyWidth(i, widths[i]);
  }
  table.classList.add("tcr-pinned");
}
function setupTableResize(plugin, table, filePath) {
  ...
  for (let i = 0; i < colCount; i++) {
    ...
    attachDragBehavior(plugin, handle, cols, headerCells, i, fileKey, colCount, applyWidth, pin);
  }
}
function attachDragBehavior(plugin, handle, cols, headerCells, colIndex, fileKey, colCount, applyWidth, pin) {
